#!/usr/bin/env python3
"""Archive an authorized X Search page via Patchright GraphQL.
...
Chrome with a persistent context. Open Search (Top / Latest / People),
capture SearchTimeline (SearchAdaptive fallback), then paginate with the
bottom cursor on the same browser context.

Writes posts.json and checkpoint.json under
./data/search-x/<slug>/ after every successful page.
...
DEFAULT_OUT = os.environ.get("SEARCH_X_OUT", str(Path.cwd() / "data" / "search-x"))
DEFAULT_PROFILE = os.environ.get("CHROME_PROFILE", str(Path.cwd() / "data" / "search-x" / "chrome-profile"))
TAB_FLAG = {"top": "", "latest": "live", "people": "user", "media": "image"}
...
def query_slug(query: str, tab: str) -> str:
    s = re.sub(r"[^A-Za-z0-9]+", "-", (query or "").strip()).strip("-").lower() or "query"
    return f"{s}-{tab}"[:80]


def search_page_url(query: str, tab: str) -> str:
    q = urllib.parse.quote(query)
    url = f"https://x.com/search?q={q}&src=typed_query"
    flag = TAB_FLAG.get(tab, "")
    if flag:
        url += f"&f={flag}"
    return url
...
def capture_live(args: argparse.Namespace) -> int:
    ...
    if tab not in TAB_FLAG:
        print(f"unknown --tab {tab}", file=sys.stderr)
        return EXIT_FAIL
    handle = query_slug(query, tab)
    ...
    profile_url = search_page_url(query, tab)
    ...
    try:
        with sync_playwright() as playwright:
            ...
            if chosen is None:
                print(
                    "no SearchTimeline or SearchAdaptive response captured",
                    file=sys.stderr,
                )
                if owned:
                    context.close()
...
def self_check() -> int:
    ...
    originals_url = (
        "https://api.x.com/graphql/QueryIdOrig/SearchTimeline"
        "?variables=%7B%22userId%22%3A%221%22%2C%22count%22%3A20%7D"
        "&features=%7B%22longform%22%3Atrue%7D"
    )
    tweets_url = "https://api.x.com/graphql/QueryIdTweets/SearchAdaptive?variables=%7B%7D"
    qid, op = parse_graphql_url(originals_url)
    if (qid, op) != ("QueryIdOrig", "SearchTimeline"):
        errors.append(f"parse SearchTimeline got {(qid, op)}")
    qid, op = parse_graphql_url(tweets_url)
    if (qid, op) != ("QueryIdTweets", "SearchAdaptive"):
        errors.append(f"parse SearchAdaptive got {(qid, op)}")
    ...
    if template is None or template.operation_name != "SearchTimeline":
        ...
        [
            (
                GraphQLTemplate(
                    ...
                    "QueryIdTweets",
                    "GET",
                    "https://api.x.com/graphql/QueryIdTweets/SearchAdaptive",
                    {},
                    {},
                    {},
                    ("authorization",),
    ...
    if fallback is None or fallback[0].operation_name != "SearchAdaptive":
        errors.append("SearchAdaptive fallback missing")

    out = Path("/tmp/search-x-self-check/example")
    ...
    prior_dir = Path("/tmp/search-x-self-check/atomic")
    ...
    resume_dir = Path("/tmp/search-x-self-check/resume")
    ...
    flush_dir = Path("/tmp/search-x-self-check/flush-pages")
    ...
    if "./data/search-x/" not in skill_text:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--query", default="", help="X Search query")
    ap.add_argument("--tab", default="latest", choices=sorted(TAB_FLAG), help="Search tab: top, latest, people, media")
    ap.add_argument(
        "--max-posts",
        type=int,
        default=0,
    ...
    ap.add_argument(
        "--out",
        default=DEFAULT_OUT,
        help="parent directory. Posts land in <out>/<query-slug-tab>/",
    )
    ap.add_argument(
        "--min-delay",
        type=float,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.self_check:
        return self_check()
    if not args.query:
        print("--query is required unless --self-check", file=sys.stderr)
        return EXIT_FAIL
    if args.max_posts < 0:
